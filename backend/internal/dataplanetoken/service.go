...
// New builds the service over the configuration database and the given configuration crypto service.
// That is the same key management protecting connection secrets, rather than a second scheme with its
// own keys to manage.
func New(crypto kmprovider.ConfigCryptoProvider) *Service {
	st := newStore()
	return &Service{
