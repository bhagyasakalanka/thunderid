...
func (s *notificationStore) listSendersByType(
	...
	return s.listSendersWithQuery(ctx, queryGetNotificationSendersByType, string(senderType),
		deployment.Resolve(ctx, s.deploymentID))
}
...
func (s *notificationStore) deleteSender(ctx context.Context, id string) error {
	...
	rowsAffected, err := dbClient.ExecuteContext(ctx, queryDeleteNotificationSender, id,
		deployment.Resolve(ctx, s.deploymentID))
	if err != nil {
