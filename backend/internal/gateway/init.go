...
func registerRoutes(mux *http.ServeMux, h *handler) {
	...
	mux.HandleFunc(middleware.WithCORS("PATCH /gateways/{id}", h.handleUpdate, itemOpts))
	...
	// Rotation is its own route rather than a field of the gateway: it returns a value the caller
	// must keep, and it must not happen by accident from sending a whole object back.
	keyOpts := middleware.CORSOptions{
		AllowedMethods:   []string{"POST"},
		AllowedHeaders:   middleware.DefaultAllowedHeaders,
		AllowCredentials: true,
		MaxAge:           600,
	}
	mux.HandleFunc(middleware.WithCORS("POST /gateways/{id}/key", h.handleRotateKey, keyOpts))
	mux.HandleFunc(middleware.WithCORS("OPTIONS /gateways/{id}/key", noContent, keyOpts))
}
