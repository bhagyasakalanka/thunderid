...
// gateway manager, which is the only caller that holds the hierarchy they describe; a console
...
func TestRecordingGatewayDetailsNeedsThePromotionScope(t *testing.T) {
	...
	for _, open := range []string{"POST /gateways/{id}/apply", "POST /gateways/{id}/revert"} {
		if routes[open] {
