...
var (
	...
	queryEnqueueJob = dbmodel.DBQuery{
		...
		Query: `INSERT INTO "DATA_PLANE_JOB" ` +
			`(DEPLOYMENT_ID, ID, DATA_PLANE_ID, GATEWAY_ID, TYPE, PAYLOAD, ENCRYPTED, STATUS) ` +
	...
	queryGetJob = dbmodel.DBQuery{
		ID: "EMQ-JOB-002",
		Query: `SELECT ID, DATA_PLANE_ID, GATEWAY_ID, TYPE, STATUS, RESULT, ERROR, ATTEMPTS ` +
	...
	queryNextPendingJob = dbmodel.DBQuery{
		ID: "EMQ-JOB-003",
		Query: `SELECT DEPLOYMENT_ID, ID, GATEWAY_ID, TYPE, PAYLOAD, ENCRYPTED ` +
