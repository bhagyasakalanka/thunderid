...
// DataPlanes resolves the data plane an gateway names.
...
// dataPlaneFor returns the data plane an gateway applies to.
//
// The error names the gateway rather than the id, because that is what the operator asked for
// and an unnamed or disconnected data plane is something they can act on.
func (s *Service) dataPlaneFor(env model.Gateway) (DataPlane, error) {
...
// DataPlaneStatus reports whether an gateway's data plane is connected.
func (s *Service) DataPlaneStatus(env model.Gateway) model.DataPlaneStatus {
	id := strings.TrimSpace(env.Target.DataPlaneID)
...
// SetDataPlaneTokenIssuer installs what mints data plane tokens. Without one, an gateway is
