...
type ServiceInterface interface {
	...
	// Register records a data plane and issues the key this control plane will present to it. The
	...
	// RotateKey issues a new key for a gateway and returns it once. The old one stops working as
	// soon as this returns, so the data plane has to be given the new one.
	RotateKey(ctx context.Context, id string) (string, *tidcommon.ServiceError)
...
func (s *service) Register(ctx context.Context,
	...
	// One data plane registers once, and its address is what decides which one it is. Two rows for
	...
	// The control plane issues the key rather than being told one: it is the side that presents it,
	// and generating it here means it is never typed, mailed or committed on the way in. It is held
	// the way every other stored credential is, encrypted with the deployment's configuration key.
	key, svcErr := s.newKey(ctx)
	if svcErr != nil {
		return nil, svcErr
	}
...
// newKey generates the token this control plane will present to a data plane.
...
func (s *service) Update(ctx context.Context, id string,
	...
	if req.Name == nil && req.BaseURL == nil && req.CACertificate == nil {
	...
	if req.BaseURL != nil {
		...
		// The address identifies the data plane, so moving one onto an address another already
		...
		if owner, ownerErr := s.store.GetByBaseURL(ctx, baseURL); ownerErr != nil {
			s.logger.Error(ctx, "Failed to check the data plane", log.Error(ownerErr))
			return nil, &tidcommon.InternalServerError
...
// RotateKey issues a new key for a gateway and returns it once.
//
// The previous key stops working the moment this returns, so the data plane drops until it is given
// the new one. That is the point: a key that may have leaked should stop working before anything
// convenient happens.
func (s *service) RotateKey(ctx context.Context, id string) (string, *tidcommon.ServiceError) {
	if strings.TrimSpace(id) == "" {
	...
	if err := s.store.Update(ctx, existing); err != nil {
		s.logger.Error(ctx, "Failed to store the gateway credential", log.Error(err))
		return "", &tidcommon.InternalServerError
	}

	s.logger.Info(ctx, "Rotated a gateway credential", log.String("gatewayId", existing.ID))
	return key, nil
}
...
func (s *service) Adopt(ctx context.Context, req RegisterRequest) *tidcommon.ServiceError {
	...
	if existing == nil {
		...
		// First time this file has been seen, so it needs a key like any other registration.
		key, svcErr := s.newKey(ctx)
		if svcErr != nil {
			return svcErr
		}
	...
	existing.BaseURL = baseURL
	// The key is deliberately left alone. These files are read on every start, so minting a new one
	// here would rotate the credential on each restart and drop the data plane until someone noticed.
	existing.CACertificate = strings.TrimSpace(req.CACertificate)
	if err := s.store.Update(ctx, existing); err != nil {
