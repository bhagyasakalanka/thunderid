...
// maxGateways is how many data planes this deployment may administer.
//
...
func (s *service) Register(ctx context.Context,
	...
	// Without an address there is no way to reach the data plane, and a registration that cannot be
	...
	// one, because the data plane was configured with it first, supplies it instead and that is
	...
	gw := &Gateway{
		...
		// The first gateway registered is the one this control plane administers directly: a
		// control plane with a single data plane has no other candidate and should not need a
		// second call to say so. A later one is marked deliberately.
...
func (s *service) Update(ctx context.Context, id string,
	...
	// same value then has to reach the data plane, which holds it as its management API key.
	//
	// An edit that carries none keeps it. Nothing is generated here, because a caller who renamed a
	// gateway did not ask for its credential to change, and a new one would stop the data plane
...
func (s *service) Adopt(ctx context.Context, req RegisterRequest) *tidcommon.ServiceError {
	...
	// minting a new one here would rotate the credential on each restart and drop the data plane
