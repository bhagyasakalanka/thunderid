...
func (s *Server) listGateways(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]interface{}{"gateways": summaries})
}
...
func (s *Server) captureSecret(w http.ResponseWriter, r *http.Request) {
	...
	// Zero deliveries is not an error: no gateway is registered for the tenant yet, and an apply
