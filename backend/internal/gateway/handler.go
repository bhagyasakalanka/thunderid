...
// handleUpdate changes a registration. What the body omits is left as it is.
func (h *handler) handleUpdate(w http.ResponseWriter, r *http.Request) {
	...
	gw, svcErr := h.service.Update(ctx, r.PathValue("id"), req)
	if svcErr != nil {
		handleError(ctx, w, svcErr)
		return
	}
...
// handleRotateKey issues a new key for a gateway and returns it once. It takes no body: there is
// nothing for a caller to supply, since the control plane is the side that generates it.
func (h *handler) handleRotateKey(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	key, svcErr := h.service.RotateKey(ctx, r.PathValue("id"))
	if svcErr != nil {
		handleError(ctx, w, svcErr)
		return
	}
	sysutils.WriteSuccessResponse(ctx, w, http.StatusOK, map[string]string{"key": key})
}
...
// decodeBody reads the request body as one JSON object into target, on the terms
// decodeRegistration describes.
