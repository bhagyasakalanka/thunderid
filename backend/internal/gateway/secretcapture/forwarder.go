...
// ServiceConfig points at the secret service a captured credential is written to. The caller reads it
// from wherever it keeps its configuration, so this package does not depend on a particular shape.
type ServiceConfig struct {
	// URL is the secret service's base URL. An empty URL means none is configured.
	URL string
	// Token authenticates to that service, when it asks for one.
	Token string
	// TimeoutSeconds bounds a single write. Defaults to ten seconds.
	TimeoutSeconds int
}

// newSecretForwarder builds a forwarder for the configured secret service. It returns nil when none
// is configured, which leaves this server with nowhere to put a captured credential.
//
...
func newSecretForwarder(cfg ServiceConfig) *secretForwarder {
...
func Select(ctx context.Context, logger *log.Logger, cfg ServiceConfig,
