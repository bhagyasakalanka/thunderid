...
// it as the gateway manager's DataPlanes.
package dataplane
...
func (p *ChannelDataPlanes) For(dataPlaneID string) (gatewayservice.DataPlane, error) {
	if p.server == nil {
...
// wire document, one as the gateway manager builds it and one as the import service reads it, and
...
// toThunderResponse converts the import service's response into the shape the gateway manager
