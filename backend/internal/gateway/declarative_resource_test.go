...
// A declaration may name the key the data plane already holds, so a data plane configured first can
...
func TestAdoptTakesTheKeyADeclarationNames(t *testing.T) {
	...
	if svcErr := svc.Adopt(context.Background(), RegisterRequest{
		Name: "dev", BaseURL: "https://dp.example.test", Key: "the-key-the-data-plane-holds",
	}); svcErr != nil {
		t.Fatalf("adopt: %v", svcErr)
	}

	stored := store.gateways[0].Key
	if stored == "the-key-the-data-plane-holds" {
...
// start, so issuing a new one here would drop the data plane at each restart.
