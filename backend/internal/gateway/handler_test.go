...
func TestHandleUpdatePassesThePartialThrough(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPatch, "/gateways/gw-1",
...
// Rotation returns the new key, and it is the only thing the response carries.
func TestHandleRotateKeyReturnsTheNewKey(t *testing.T) {
	svc := &stubService{rotatedKey: "issued-key"}
	request := httptest.NewRequest(http.MethodPost, "/gateways/gw-1/key", nil)
	request.SetPathValue("id", testGatewayID)
	recorder := httptest.NewRecorder()
	newHandler(svc).handleRotateKey(recorder, request)

	if recorder.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", recorder.Code, recorder.Body.String())
	}
	if svc.rotatedID != testGatewayID {
		t.Fatalf("the id did not reach the service: %q", svc.rotatedID)
	}
	if !strings.Contains(recorder.Body.String(), "issued-key") {
		t.Fatalf("the new key was not returned: %s", recorder.Body.String())
	}
}
