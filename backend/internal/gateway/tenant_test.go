...
func TestRegistryGivesEachDeploymentItsOwnManager(t *testing.T) {
	...
	if a == b {
		t.Fatal("two deployments must not share one gateway manager")
	}
...
func TestRegistryRefusesARequestWithNoDeployment(t *testing.T) {
	...
	// Serving this from a default store would hand one deployment another's gateways.
...
func TestInitializeRegistersTheSecretRoutes(t *testing.T) {
	...
	for _, route := range []struct{ method, path string }{
		{http.MethodGet, "/gateways/env-1/secrets"},
		{http.MethodPut, "/gateways/env-1/secrets/APP_A_CLIENT_SECRET"},
		{http.MethodPost, "/gateways/env-1/secrets/APP_A_CLIENT_SECRET/regenerate"},
	} {
