...
// Package auth carries the caller's bearer token across a request so the gateway manager can
...
// the gateway manager makes are performed as the caller, so without this every one of them
