...
// TestMain configures the bound these tests run under.
//
// Nothing in the code supplies a fallback any more: config/default.json carries the number, so a
// server always reads one. These tests stand in for that file, which is why a test that never
// mentions the bound still has one.
func TestMain(m *testing.M) {
	configureGatewayBound(shippedGatewayBound)
	code := m.Run()
	config.ResetServerRuntime()
	os.Exit(code)
}

// shippedGatewayBound is what config/default.json carries, and what a deployment that configures
// nothing administers.
const shippedGatewayBound = 1

// configureGatewayBound puts a bound in the runtime configuration, as loading a config file would.
func configureGatewayBound(bound int) {
	config.ResetServerRuntime()
	_ = config.InitializeServerRuntime("", &config.Config{
		Gateway: config.GatewayConfig{MaxGateways: bound},
	})
}

// allowSeveralGateways lifts the registration bound for a test and restores it afterwards. The
// shipped bound is one, which would otherwise refuse a second registration before the rule under
// test is reached.
func allowSeveralGateways(t *testing.T) {
	t.Helper()
	configureGatewayBound(5)
	t.Cleanup(func() { configureGatewayBound(shippedGatewayBound) })
}
...
// An edit that carries no key leaves the stored one exactly as it was. This is what makes renaming
// a gateway safe: its data plane keeps answering to the credential it already holds.
func TestUpdateWithoutAKeyLeavesItAlone(t *testing.T) {
	allowSeveralGateways(t)
	cmodels.SetConfigCryptoProvider(reversingCrypto{})
	t.Cleanup(func() { cmodels.SetConfigCryptoProvider(nil) })
	...
	registered, svcErr := svc.Register(ctx, RegisterRequest{
		Name: "dev", BaseURL: "https://dp.example.test", Key: "the-stored-key",
	})
	if svcErr != nil {
		t.Fatalf("register: %v", svcErr)
	}
	...
	renamed := "renamed"
	if _, svcErr := svc.Update(ctx, registered.ID, UpdateRequest{Name: &renamed}); svcErr != nil {
		t.Fatalf("update: %v", svcErr)
	}

	if store.gateways[0].Key != before {
		t.Fatal("an edit that carried no key changed the stored one")
	}
}

// An edit that carries a key replaces the stored one, which is how a credential is rotated now that
// there is no route of its own for it.
func TestUpdateWithAKeyReplacesIt(t *testing.T) {
	allowSeveralGateways(t)
	cmodels.SetConfigCryptoProvider(reversingCrypto{})
	t.Cleanup(func() { cmodels.SetConfigCryptoProvider(nil) })
	...
	replacement := "the-second-key"
	updated, svcErr := svc.Update(ctx, registered.ID, UpdateRequest{Key: &replacement})
	if svcErr != nil {
		t.Fatalf("update: %v", svcErr)
	}

	if store.gateways[0].Key == before {
		t.Fatal("an edit carrying a key did not replace the stored one")
	}
	if store.gateways[0].Key == replacement {
		t.Fatal("the replacement was stored without being encrypted")
	}
	if updated.Key != "" {
