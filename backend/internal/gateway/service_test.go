...
func (f *fakeStore) GetByBaseURL(_ context.Context, baseURL string) (*Gateway, error) {
	if f.err != nil {
		return nil, f.err
	}
	for i := range f.gateways {
		if f.gateways[i].BaseURL == baseURL {
			found := f.gateways[i]
			return &found, nil
		}
	}
...
func TestRegisterStoresTheCredentialEncrypted(t *testing.T) {
	store := &fakeStore{}
	reg, svcErr := newTestService(t, store).Register(context.Background(), validRequest("dev"))
	if svcErr != nil {
		t.Fatalf("register: %v", svcErr)
	}
	if reg.Key == "" {
		t.Fatal("registration must hand back the key it issued")
	}
	...
	if stored == reg.Key {
		t.Fatal("the issued key was stored in the clear")
	}
	...
	if err != nil || value != reg.Key {
		t.Fatalf("expected the credential to decrypt to what was issued, got %q err=%v", value, err)
	}
}

// A credential must never travel back out of the API.
// Registration is the one call that hands the key back, because it is the only chance to put it on
// the data plane. Nothing else does.
func TestOnlyRegistrationReturnsTheCredential(t *testing.T) {
	store := &fakeStore{}
	svc := newTestService(t, store)
	created, _ := svc.Register(context.Background(), validRequest("dev"))
	if created.Key == "" {
		t.Error("register should return the key it issued")
	}
	if created.Gateway.Key != "" {
		t.Error("the stored credential traveled with the gateway")
	}
...
func TestRegisterRefusesAnAddressAlreadyRegistered(t *testing.T) {
	cmodels.SetConfigCryptoProvider(reversingCrypto{})
	t.Cleanup(func() { cmodels.SetConfigCryptoProvider(nil) })
	...
	store := &fakeStore{gateways: []Gateway{
		{ID: "gw-1", Name: "production", BaseURL: "https://dp.example.test"},
	}}
...
func TestAdoptRefusesAnAddressOwnedByAnotherGateway(t *testing.T) {
	allowSeveralGateways(t)

	store := &fakeStore{gateways: []Gateway{
		{ID: "gw-1", Name: "dev", BaseURL: "https://dp-one"},
		{ID: "gw-2", Name: "staging", BaseURL: "https://dp-two"},
	}}

	svcErr := newTestService(t, store).Adopt(context.Background(), RegisterRequest{
		Name:    "staging",
		BaseURL: "https://dp-one", // already answered by gw-1
	})
...
func TestAdoptKeepsItsOwnAddress(t *testing.T) {
	allowSeveralGateways(t)

	store := &fakeStore{gateways: []Gateway{{ID: "gw-1", Name: "dev", BaseURL: "https://dp"}}}
...
func TestAdoptRefusesADeclarationWithoutAName(t *testing.T) {
	...
	svcErr := newService(&fakeStore{}).Adopt(context.Background(), RegisterRequest{BaseURL: "https://dp"})
...
// registerOne registers a gateway and returns it, so an update has something to change.
func registerOne(t *testing.T, store storeInterface) (ServiceInterface, *Gateway) {
	t.Helper()
	svc := newTestService(t, store)
	reg, svcErr := svc.Register(context.Background(), validRequest("dev"))
	if svcErr != nil {
		t.Fatalf("register: %v", svcErr)
	}
	return svc, &reg.Gateway
}

func strptr(s string) *string { return &s }

func TestUpdateChangesOnlyWhatTheRequestNames(t *testing.T) {
	store := &fakeStore{}
	svc, gw := registerOne(t, store)

	updated, svcErr := svc.Update(context.Background(), gw.ID, UpdateRequest{
		BaseURL: strptr("https://moved.example.test"),
	})
	if svcErr != nil {
		t.Fatalf("update: %v", svcErr)
	}
	if updated.BaseURL != "https://moved.example.test" {
		t.Fatalf("baseUrl not applied: %q", updated.BaseURL)
	}
	// Everything the request did not name is left as it was.
	if updated.Name != "dev" {
		t.Fatalf("name should be untouched, got %q", updated.Name)
	}
	if updated.ID == "" {
		t.Fatal("the registration should keep its id")
	}
}

// The key is never handed back, by update as by every other read.
func TestUpdateDoesNotReturnTheKey(t *testing.T) {
	store := &fakeStore{}
	svc, gw := registerOne(t, store)

	updated, svcErr := svc.Update(context.Background(), gw.ID, UpdateRequest{Name: strptr("prod")})
	if svcErr != nil {
		t.Fatalf("update: %v", svcErr)
	}
	if updated.Key != "" {
		t.Fatal("the key must not be returned")
	}
	// It is still stored, though: an update is not a way to clear it.
	if store.gateways[0].Key == "" {
		t.Fatal("the stored key must survive an update")
	}
}

func TestUpdateRefusesWhatItCannotApply(t *testing.T) {
	store := &fakeStore{}
	svc, gw := registerOne(t, store)

	if _, svcErr := svc.Update(context.Background(), gw.ID, UpdateRequest{}); svcErr == nil ||
		svcErr.Code != ErrorNothingToUpdate.Code {
		t.Fatalf("an empty update must be refused, got %v", svcErr)
	}
	if _, svcErr := svc.Update(context.Background(), gw.ID, UpdateRequest{
		BaseURL: strptr("not a url"),
	}); svcErr == nil || svcErr.Code != ErrorInvalidBaseURL.Code {
		t.Fatalf("an unusable address must be refused, got %v", svcErr)
	}
	if _, svcErr := svc.Update(context.Background(), gw.ID, UpdateRequest{
		Name: strptr("   "),
	}); svcErr == nil || svcErr.Code != ErrorGatewayNameRequired.Code {
		t.Fatalf("an empty name must be refused, got %v", svcErr)
	}
	if _, svcErr := svc.Update(context.Background(), "missing", UpdateRequest{
		Name: strptr("x"),
	}); svcErr == nil || svcErr.Code != ErrorGatewayNotFound.Code {
		t.Fatalf("an unknown gateway must be refused, got %v", svcErr)
	}
}

// A name identifies a gateway to the files that declare one, so two may not share it. Keeping its
// own name is not a clash.
func TestUpdateGuardsTheNameWithoutClashingWithItself(t *testing.T) {
	store := &fakeStore{}
	svc, first := registerOne(t, store)
	second, svcErr := svc.Register(context.Background(), RegisterRequest{
		Name: "prod", BaseURL: "https://other.example.test",
	})
	if svcErr != nil {
		t.Skipf("a second registration is not allowed here: %v", svcErr)
	}

	if _, err := svc.Update(context.Background(), second.ID, UpdateRequest{
		Name: strptr("dev"),
	}); err == nil || err.Code != ErrorGatewayNameTaken.Code {
		t.Fatalf("taking another gateway's name must be refused, got %v", err)
	}
	if _, err := svc.Update(context.Background(), first.ID, UpdateRequest{
		Name: strptr("dev"),
	}); err != nil {
		t.Fatalf("keeping its own name must be allowed, got %v", err)
	}
}

func TestRotateKeyIssuesANewCredential(t *testing.T) {
	store := &fakeStore{}
	svc, gw := registerOne(t, store)
	before := store.gateways[0].Key

	key, svcErr := svc.RotateKey(context.Background(), gw.ID)
	if svcErr != nil {
		t.Fatalf("rotate: %v", svcErr)
	}
	if key == "" {
		t.Fatal("rotation must return the new key")
	}
	after := store.gateways[0].Key
	if after == before {
		t.Fatal("the stored credential should have changed")
	}
	if after == key {
		t.Fatal("the new key was stored in the clear")
	}

	// What is returned is what is stored, or the data plane would be given a key that does not work.
	props, err := cmodels.DeserializePropertiesFromJSON(after)
	if err != nil || len(props) != 1 {
		t.Fatalf("stored credential is unreadable: %v", err)
	}
	value, err := props[0].GetValue()
	if err != nil || value != key {
		t.Fatalf("stored credential does not match what was returned: %q (%v)", value, err)
	}
}

// Every registration gets its own key, so one leaking says nothing about the next.
func TestEachIssuedKeyIsDifferent(t *testing.T) {
	store := &fakeStore{}
	svc, gw := registerOne(t, store)

	first, svcErr := svc.RotateKey(context.Background(), gw.ID)
	if svcErr != nil {
		t.Fatalf("rotate: %v", svcErr)
	}
	second, svcErr := svc.RotateKey(context.Background(), gw.ID)
	if svcErr != nil {
		t.Fatalf("rotate: %v", svcErr)
	}
	if first == second {
		t.Fatal("two rotations produced the same key")
	}
}

func TestRotateKeyRefusesAnUnknownGateway(t *testing.T) {
	store := &fakeStore{}
	svc, _ := registerOne(t, store)

	if _, svcErr := svc.RotateKey(context.Background(), "missing"); svcErr == nil ||
		svcErr.Code != ErrorGatewayNotFound.Code {
		t.Fatalf("an unknown gateway must be refused, got %v", svcErr)
	}
	if _, svcErr := svc.RotateKey(context.Background(), "  "); svcErr == nil ||
		svcErr.Code != ErrorInvalidGatewayID.Code {
		t.Fatalf("an empty id must be refused, got %v", svcErr)
	}
}
