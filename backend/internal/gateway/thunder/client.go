...
// Package thunder is an HTTP client for the ThunderID management APIs used by the gateway
...
type gatewayVariablesResponse struct {
	Variables map[string]string `json:"variables"`
}

// GatewayVariables returns the non-secret variable values configured for one gateway, such as
// redirect URLs. These are authoritative for an apply: they are what the operator set for that
// gateway, so they override whatever the export happened to emit. A 403 or 404 is treated as an
// empty map so capture still works against servers without the module.
func (c *Client) GatewayVariables(ctx context.Context, envID string) (map[string]string, error) {
	var resp gatewayVariablesResponse
	err := c.do(ctx, http.MethodGet, "/gateways/"+url.PathEscape(envID)+"/variables/resolve", nil, &resp)
	if err != nil {
		if isStatus(err, http.StatusForbidden) || isStatus(err, http.StatusNotFound) {
...
// ImportRequest is the payload for POST /import. Deletions is the gateway service's extension for
