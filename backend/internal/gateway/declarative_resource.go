...
type declaredGateway struct {
	...
	// Key is the token this control plane presents to that data plane.
	//
	// Optional, and what makes a data plane configured with a key first something this file can
	...
	// across later reads.
	//
	// A literal key here is a data plane credential in source control, so write it as an
