...
func (s *resourceStore) GetResourceServer(ctx context.Context, id string) (providers.ResourceServer, error) {
	...
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryGetResourceServerByID, id,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *resourceStore) GetResourceServerList(
	...
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryGetResourceServerList, limit, offset,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *resourceStore) GetResourceServerListCount(ctx context.Context) (int, error) {
	var count int
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryGetResourceServerListCount,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *resourceStore) CheckResourceServerNameExists(ctx context.Context, name string) (bool, error) {
	var exists bool
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryCheckResourceServerNameExists, name,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *resourceStore) GetResource(ctx context.Context, id string, resServerID string) (providers.Resource, error) {
	...
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryGetResourceByID, id, resServerID,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *resourceStore) GetResourceListByParent(
	...
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		...
		if parentID == nil {
			...
			results, err = dbClient.QueryContext(
				ctx,
				queryGetResourceListByParent, resServerID, *parentID, limit, offset,
				deployment.Resolve(ctx, s.deploymentID),
			)
		}
...
func (s *resourceStore) GetResourceListCount(ctx context.Context, resServerID string) (int, error) {
	var count int
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryGetResourceListCount, resServerID,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *resourceStore) DeleteResource(ctx context.Context, id string, resServerID string) error {
	return s.withDBClient(func(dbClient provider.DBClientInterface) error {
		_, err := dbClient.ExecuteContext(ctx, queryDeleteResource, id, resServerID,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *resourceStore) CheckResourceHandleExists(
	...
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		...
		if parentID == nil {
			results, err = dbClient.QueryContext(
				ctx,
				queryCheckResourceHandleExistsUnderNullParent, resServerID, handle,
				deployment.Resolve(ctx, s.deploymentID),
			)
		} else {
			results, err = dbClient.QueryContext(
...
func (s *resourceStore) CheckResourceHasDependencies(ctx context.Context, resID string) (bool, error) {
	var hasDeps bool
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryCheckResourceHasDependencies, resID,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
			return fmt.Errorf("failed to check dependencies: %w", err)
		}
...
func (s *resourceStore) GetActionListCount(
	...
	err := s.withDBClient(func(dbClient provider.DBClientInterface) error {
		...
		if kind != "" {
			results, err = dbClient.QueryContext(
				ctx, queryGetActionListCountByKind, resServerID, resID, string(kind),
				deployment.Resolve(ctx, s.deploymentID),
			)
		} else {
			results, err = dbClient.QueryContext(
