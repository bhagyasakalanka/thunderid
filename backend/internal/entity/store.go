...
func (es *entityDBStore) CreateEntity(ctx context.Context, entity providers.Entity,
	...
	_, err = dbClient.ExecuteContext(
		...
		QueryCreateEntity,
		entity.ID,
		deployment.Resolve(ctx, es.deploymentID),
...
func (es *entityDBStore) GetEntity(ctx context.Context, id string) (providers.Entity, error) {
	...
	results, err := dbClient.QueryContext(ctx, QueryGetEntityByID, id, deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) GetEntityWithCredentials(ctx context.Context, id string) (
	*entityWithCredentials, error) {
	...
	results, err := dbClient.QueryContext(ctx, QueryGetEntityWithCredentials, id,
		deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) UpdateEntity(ctx context.Context, entity *providers.Entity) error {
	...
	_, err = dbClient.ExecuteContext(ctx, QueryDeleteIdentifiersByEntity, entity.ID,
		deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) UpdateAttributes(ctx context.Context, entityID string, attributes json.RawMessage) error {
	return es.updateAttributeSet(ctx, entityID, QueryUpdateAttributes, QueryDeleteAttributeIdentifiersByEntity,
		attributes, nil)
}
...
func (es *entityDBStore) UpdateSystemAttributes(ctx context.Context, entityID string,
	attrs json.RawMessage) error {
	return es.updateAttributeSet(ctx, entityID, QueryUpdateSystemAttributes, QueryDeleteSystemIdentifiersByEntity,
		nil, attrs)
}

// updateAttributeSet writes one of the two attribute columns and rebuilds the identifiers it sources.
//
// The identifiers are deleted and re-synced rather than diffed, because an attribute that no longer
// appears has to stop resolving, and a diff would have to derive that from the previous value the
// caller did not send.
func (es *entityDBStore) updateAttributeSet(ctx context.Context, entityID string, updateQuery,
	deleteIdentifiersQuery dbmodel.DBQuery, attributes, systemAttributes json.RawMessage) error {
	dbClient, err := es.dbProvider.GetEntityDBClient()
	if err != nil {
		return fmt.Errorf("failed to get database client: %w", err)
	}
	...
	rowsAffected, err := dbClient.ExecuteContext(ctx, updateQuery,
		entityID, string(payload), time.Now().UTC(), deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
	...
	if _, err = dbClient.ExecuteContext(ctx, deleteIdentifiersQuery,
		entityID, deployment.Resolve(ctx, es.deploymentID)); err != nil {
		return fmt.Errorf("failed to delete identifiers: %w", err)
	}

	if err = es.syncAttributeIdentifiers(ctx, entityID, attributes, systemAttributes,
		es.indexedAttributes); err != nil {
		return fmt.Errorf("failed to sync identifiers: %w", err)
	}
...
func (es *entityDBStore) UpdateCredentials(ctx context.Context, entityID string,
	...
	rowsAffected, err := dbClient.ExecuteContext(ctx, QueryUpdateCredentials,
		entityID, string(creds), time.Now().UTC(), deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) DeleteEntity(ctx context.Context, id string) error {
	...
	rowsAffected, err := dbClient.ExecuteContext(ctx, QueryDeleteEntity, id, deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
		return fmt.Errorf("failed to execute query: %w", err)
	}
	...
	if _, err = dbClient.ExecuteContext(ctx, QueryDeleteIdentifiersByEntity, id,
		deployment.Resolve(ctx, es.deploymentID)); err != nil {
...
func (es *entityDBStore) syncAttributeIdentifiers(ctx context.Context, entityID string,
	...
	query, args, err := prepareIdentifierQuery(entityID, attributes, systemAttributes, indexedAttrs,
		deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
		return err
	}
...
func (es *entityDBStore) IdentifyEntity(ctx context.Context,
	...
	identifyQuery, args, err := buildIdentifyQueryFromIdentifiers(filters, deployment.Resolve(ctx, es.deploymentID))
	if err == nil {
	...
	if len(indexedFilters) > 0 && len(nonIndexedFilters) > 0 {
		...
		fallbackQuery, fallbackArgs, err = buildIdentifyQueryHybrid(indexedFilters, nonIndexedFilters,
			deployment.Resolve(ctx, es.deploymentID))
		if err != nil {
		...
		fallbackQuery, fallbackArgs, err = buildIdentifyQuery(filters, deployment.Resolve(ctx, es.deploymentID))
		if err != nil {
...
func (es *entityDBStore) SearchEntities(ctx context.Context,
	...
	searchQuery, args, err := buildEntityListQuery(
		"", filters, serverconst.MaxPageSize, 0, deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) GetEntityListCount(ctx context.Context, category string,
	...
	countQuery, args, err := buildEntityCountQuery(category, filters, deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) GetEntityList(ctx context.Context, category string,
	...
	listQuery, args, err := buildEntityListQuery(category, filters, limit, offset,
		deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) GetEntityListCountByOUIDs(ctx context.Context, category string,
	...
	countQuery, args, err := buildEntityCountQueryByOUIDs(category, ouIDs, filters,
		deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
		return 0, fmt.Errorf("failed to build count query: %w", err)
	}
...
func (es *entityDBStore) GetEntityListByOUIDs(ctx context.Context, category string,
	...
	listQuery, args, err := buildEntityListQueryByOUIDs(category, ouIDs, filters, limit, offset,
		deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
		return nil, fmt.Errorf("failed to build list query: %w", err)
	}
...
func (es *entityDBStore) ValidateEntityIDs(ctx context.Context, entityIDs []string) ([]string, error) {
	...
	query, args, err := buildBulkEntityExistsQuery(entityIDs, deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) GetEntitiesByIDs(ctx context.Context, entityIDs []string) ([]providers.Entity, error) {
	...
	for start := 0; start < len(entityIDs); start += batchSize {
		...
		query, args, err := buildGetEntitiesByIDsQuery(chunk, deployment.Resolve(ctx, es.deploymentID))
		if err != nil {
...
func (es *entityDBStore) ValidateEntityIDsInOUs(
	...
	query, args, err := buildBulkEntityExistsQueryInOUs(entityIDs, ouIDs, deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) GetGroupCountForEntity(ctx context.Context, entityID string) (int, error) {
	...
	countResults, err := dbClient.QueryContext(ctx, QueryGetGroupCountForEntity, entityID,
		deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
...
func (es *entityDBStore) GetEntityGroups(
	...
	results, err := dbClient.QueryContext(ctx, QueryGetGroupsForEntity,
		entityID, limit, offset, deployment.Resolve(ctx, es.deploymentID))
	if err != nil {
