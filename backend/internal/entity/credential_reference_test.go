...
func TestCredentialReference_ResolvesAPromotedCredentialFromTheSecretProvider(t *testing.T) {
	...
	ref, usable := credentialReference(StoredCredential{Value: "sec:MY_APP_CLIENT_SECRET"})
	if !usable {
...
func TestCredentialReference_RejectsAnUnresolvableReference(t *testing.T) {
	...
	if _, usable := credentialReference(StoredCredential{Value: "sec:NOT_IN_THE_PROVIDER"}); usable {
...
func TestHashPlaintextCredentials_KeepsAReferenceAsItIs(t *testing.T) {
	...
	stored, err := svc.hashPlaintextCredentials(json.RawMessage(`{"clientSecret":"sec:MY_APP_CLIENT_SECRET"}`))
	if err != nil {
	...
	// Hashing the reference text would store the hash of "sec:..." and reject every authentication.
	if got := creds["clientSecret"][0].Value; got != "sec:MY_APP_CLIENT_SECRET" {
