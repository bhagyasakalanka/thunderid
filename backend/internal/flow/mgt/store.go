...
func (s *flowStore) ListFlows(ctx context.Context, limit, offset int, flowType string) (
	...
	err := s.withDBClientContext(ctx, func(dbClient provider.DBClientInterface) error {
		...
		if flowType != "" {
			countResults, err = dbClient.QueryContext(ctx, queryCountFlowsWithType, flowType,
				deployment.Resolve(ctx, s.deploymentID))
			if err != nil {
				return fmt.Errorf("failed to count flows: %w", err)
			}

			results, err = dbClient.QueryContext(ctx, queryListFlowsWithType, flowType,
				deployment.Resolve(ctx, s.deploymentID), limit, offset)
			if err != nil {
			...
			results, err = dbClient.QueryContext(ctx, queryListFlows,
				deployment.Resolve(ctx, s.deploymentID), limit, offset)
			if err != nil {
				return fmt.Errorf("failed to list flows: %w", err)
			}
...
func (s *flowStore) ListActiveFlowsWithNodes(ctx context.Context) ([]*providers.CompleteFlowDefinition, error) {
	...
	err := s.withDBClientContext(ctx, func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryListActiveFlowsWithNodes,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *flowStore) UpdateFlow(ctx context.Context, flowID string, flow *FlowDefinition) (
	*providers.CompleteFlowDefinition, error) {
	...
	err = s.withDBClientContext(ctx, func(dbClient provider.DBClientInterface) error {
		...
		_, err = dbClient.ExecuteContext(ctx, queryUpdateFlow, flowID, flow.Name, newVersion,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *flowStore) ListFlowVersions(ctx context.Context, flowID string) ([]BasicFlowVersion, error) {
	...
	err := s.withDBClientContext(ctx, func(dbClient provider.DBClientInterface) error {
		results, err := dbClient.QueryContext(ctx, queryListFlowVersions, flowID,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *flowStore) RestoreFlowVersion(ctx context.Context, flowID string, version int) (
	*providers.CompleteFlowDefinition, error) {
	err := s.withDBClientContext(ctx, func(dbClient provider.DBClientInterface) error {
		...
		versionResults, err := dbClient.QueryContext(ctx, queryGetFlowVersion, flowID, version,
			deployment.Resolve(ctx, s.deploymentID))
		if err != nil {
...
func (s *flowStore) pushToVersionStack(ctx context.Context, dbClient provider.DBClientInterface,
	...
	countResults, err := dbClient.QueryContext(ctx, queryCountFlowVersions, flowID,
		deployment.Resolve(ctx, s.deploymentID))
	if err != nil {
	...
	if versionCount > s.maxVersionHistory {
		if _, err := dbClient.ExecuteContext(ctx, queryDeleteOldestVersion, flowID,
