...
func (s *service) Author(ctx context.Context, resourceType, name, payload string) (*Resource, error) {
	...
	if err := checkWellFormed(payload); err != nil {
		return nil, err
	}

	// The rules the resource type owns, which both planes apply.
	if resourcevalidation.Supports(resourceType) {
		if err := resourcevalidation.Validate(ctx, resourceType, []byte(payload)); err != nil {
			return nil, err
		}
	}
...
// checkWellFormed confirms the payload is a JSON document rather than something else entirely.
//
// A document that is not JSON is a mistake this plane can see on its own, and catching it here means
// the failure names the document rather than surfacing as a parse error on a gateway much later.
func checkWellFormed(payload string) error {
	if strings.TrimSpace(payload) == "" {
		return fmt.Errorf("a payload is required")
	}
	var document map[string]interface{}
	if err := json.Unmarshal([]byte(payload), &document); err != nil {
		return fmt.Errorf("the payload is not a JSON document: %w", err)
	}
	if len(document) == 0 {
		return fmt.Errorf("the payload describes nothing")
	}
	return nil
}
