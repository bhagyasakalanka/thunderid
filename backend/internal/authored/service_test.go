...
// The document is kept as it was written. A control plane is not running it, so nothing here needs
// to be resolved, and resolving it would bind the document to this plane's values.
func TestAuthor_KeepsTheDocumentAsWritten(t *testing.T) {
	...
	if stored.Payload != document {
		t.Errorf("the payload should be stored verbatim,\n got %s\nwant %s", stored.Payload, document)
	}
...
func TestAuthor_ReplacesADocumentOfTheSameTypeAndName(t *testing.T) {
	...
	if stored.Payload != edited {
		t.Error("the later document should stand")
	}
}
