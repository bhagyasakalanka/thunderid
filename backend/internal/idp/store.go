...
func (s *idpStore) GetIdentityProviderListCount(ctx context.Context) (int, error) {
	...
	results, err := dbClient.QueryContext(ctx, queryGetIdentityProviderListCount,
...
func (s *idpStore) DeleteIdentityProvider(ctx context.Context, id string) error {
	...
	rowsAffected, err := dbClient.ExecuteContext(ctx, queryDeleteIdentityProviderByID, id,
		deployment.Resolve(ctx, s.deploymentID))
	if err != nil {
		return fmt.Errorf("failed to execute query: %w", err)
	}
