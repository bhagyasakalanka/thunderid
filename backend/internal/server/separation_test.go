...
func TestOnlyTheDataPlaneMountsOpenID4VPRoutes(t *testing.T) {
	...
	for _, pkg := range callers {
		if pkg != filepath.Join("..", "..", "internal", "dataplane") {
...
// packagesCalling returns the packages whose non-test sources contain call.
//
// The walk starts at the backend root rather than at internal, so that a main package is searched
// too. A control plane that mounted the routes directly from cmd/cp would otherwise satisfy this
// check by not being looked at, which is the one place the answer matters most.
