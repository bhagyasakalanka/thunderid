...
func (s *organizationUnitStore) GetOrganizationUnit(
	...
	results, err := dbClient.QueryContext(ctx, queryGetOrganizationUnitByID, id,
		deployment.Resolve(ctx, s.deploymentID))
	if err != nil {
...
func (s *organizationUnitStore) GetOrganizationUnitByHandle(
	...
	if parent == nil {
		results, err = dbClient.QueryContext(ctx, queryGetRootOrganizationUnitByHandle, handle,
			deployment.Resolve(ctx, s.deploymentID))
	} else {
...
func (s *organizationUnitStore) IsOrganizationUnitExists(ctx context.Context, id string) (bool, error) {
	...
	results, err := dbClient.QueryContext(ctx, queryCheckOrganizationUnitExists, id,
		deployment.Resolve(ctx, s.deploymentID))
	if err != nil {
