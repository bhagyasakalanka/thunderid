...
// NewWithStore builds an enabled registry over the given store. It exists so the domain services that
// mark their listings as read only can be tested without a database behind them.
func NewWithStore(store StoreInterface) *Registry {
	return &Registry{enabled: true, store: store}
}
...
// issue a lookup per row. A lookup failure is reported rather than reduced to an empty set, because
// an empty set reads as "nothing is managed" and offers edit controls the write path then refuses.
func (r *Registry) ManagedIDs(ctx context.Context, resourceType string) (map[string]bool, error) {
	if !r.Enabled() {
		return nil, nil
	}
	return r.store.ManagedIDs(ctx, resourceType)
}
