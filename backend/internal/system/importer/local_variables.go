...
//
// A credential held as a hash keeps its reference rather than its value, because there is nothing
// readable to substitute and the reference is what authentication resolves later. Only a readable
...
func fillSecretPlaceholders(ctx context.Context, content string,
	...
	for _, name := range referencedVariables(scalarTemplateVarPattern, content) {
		...
		if _, found, err := resolver.ResolveHash(ctx, secretresolver.Prefix+name); err == nil && found {
			// A credential held as a hash keeps its reference rather than its value: there is nothing
			// readable to substitute, and the reference is what authentication resolves later.
