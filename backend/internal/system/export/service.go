...
func (es *exportService) exportResourcesWithExporter(
	...
	for _, resourceID := range resourceIDList {
		...
		if clash, previous := claimedElsewhere(templateContent, variableOwners, owner); clash != "" {
			// The name itself is not logged: it is derived from the resource name, which for a user is
			// their username.
...
// claimedVariablePattern matches the template actions an exporter may write into a resource itself.
//
// It is deliberately broader than templateVariablePattern, which the environment file is built from
// and whose matching must not change: a template action is valid with surrounding spaces or a
// trimming marker, so a placeholder spelled that way still claims its name here rather than slipping
// past the check.
