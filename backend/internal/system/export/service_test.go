...
func (suite *ExportServiceTestSuite) TestExportEntityTypes_Wildcard() {
	...
	suite.mockEntityTypeService.EXPECT().
		GetEntityTypeList(mock.Anything, entitytype.TypeCategoryUser, 100, 0, mock.Anything).
		Return(mockSchemaList, nil)
	// Agent types export through the same resource type; this fixture has none.
	suite.mockEntityTypeService.EXPECT().
		GetEntityTypeList(mock.Anything, entitytype.TypeCategoryAgent, 100, 0, mock.Anything).
		Return(&entitytype.EntityTypeListResponse{}, nil)
	suite.mockEntityTypeService.EXPECT().
		GetEntityType(mock.Anything, mock.Anything, "schema1", mock.Anything).
		Return(mockSchema1, nil)
	suite.mockEntityTypeService.EXPECT().
