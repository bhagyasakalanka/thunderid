...
// A process reading the id from a token claim does not fall back to the configured identifier: a
// request with no deployment of its own would otherwise be served against whichever one the server
// happened to be configured with.
func TestResolve_DoesNotFallBackWhenReadingFromATokenClaim(t *testing.T) {
	if err := UseTokenClaim("deploymentId"); err != nil {
		t.Fatalf("expected the claim to be accepted, got %v", err)
	}
	t.Cleanup(UseServerIdentifier)

	if got := Resolve(context.Background(), "configured-id"); got != "" {
		t.Fatalf("expected no deployment id, got %q", got)
	}
...
// Naming no claim is refused, so a process cannot end up expecting one it never named.
func TestUseTokenClaim_RequiresAClaimName(t *testing.T) {
	if err := UseTokenClaim("  "); err == nil {
		t.Fatal("expected an empty claim name to be refused")
	}
	if _, reads := TokenClaim(); reads {
		t.Fatal("expected the process to still read the configured identifier")
	}
}
