...
func (s *entityTypeStore) GetEntityTypeListCount(ctx context.Context, category TypeCategory) (int, error) {
	...
	countResults, err := dbClient.QueryContext(ctx, queryGetEntityTypeCount, string(category),
...
func (s *entityTypeStore) GetEntityTypeList(ctx context.Context, category TypeCategory,
	...
	results, err := dbClient.QueryContext(ctx, queryGetEntityTypeList, limit, offset,
		deployment.Resolve(ctx, s.deploymentID), string(category))
	if err != nil {
...
func (s *entityTypeStore) GetEntityTypeByID(ctx context.Context, category TypeCategory,
	...
	results, err := dbClient.QueryContext(ctx, queryGetEntityTypeByID, schemaID,
		deployment.Resolve(ctx, s.deploymentID), string(category))
	if err != nil {
...
func (s *entityTypeStore) GetEntityTypeByName(ctx context.Context, category TypeCategory,
	...
	results, err := dbClient.QueryContext(ctx, queryGetEntityTypeByName, name,
		deployment.Resolve(ctx, s.deploymentID), string(category))
	if err != nil {
		return EntityType{}, fmt.Errorf("failed to execute query: %w", err)
	}
...
func (s *entityTypeStore) DeleteEntityTypeByID(ctx context.Context, category TypeCategory,
	...
	rowsAffected, err := dbClient.ExecuteContext(ctx, queryDeleteEntityTypeByID, schemaID,
		deployment.Resolve(ctx, s.deploymentID),
		string(category))
	if err != nil {
