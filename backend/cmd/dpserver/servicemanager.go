...
func registerServices(mux *http.ServeMux, cacheManager cache.CacheManagerInterface) (
	...
	*secretstore.Store) {
	...
	fatalOnError(ctx, logger, err, "Failed to initialize authn provider manager")
	...
	// Resolves a request that names no resource server against the deployment's default, so consumers
	// depend on the provider rather than on the server config service.
	...
	// Initialized after the OAuth services because credential issuance validates the presented token.
...
// service. The OpenID4VCI issuer is initialized later, once the services it depends on exist.
...
// sessionCriteriaRevoker adapts the OAuth revocation service to what a session logout needs.
