...
| `gateway.max_gateways` | `1` | Guards how many data planes this deployment administers, through the Gateway API (`POST /gateways`) or a gateway declared in a file. A standalone deployment pairs with one; raise it only where something else manages the fleet. A registration beyond the number is refused with `409`. This is a guard rather than a guarantee: two registrations arriving at the same moment can leave one more gateway than configured, so do not rely on it as a licensing or security boundary. |
...
| `server.security.rest.audience` | `""` (unset) | Audience ([RFC 8707](https://datatracker.ietf.org/doc/html/rfc8707) resource indicator) a self-issued token must carry to be accepted by the REST API gate, normally the identifier of the resource server your management tokens are bound to. While unset the audience is **not** checked and REST authorizes by scope alone, so this is opt-in. When set, it must be an absolute URI without a fragment; an explicitly empty value fails startup rather than silently disabling the check |
| `server.security.mcp.audience` | `""` (derived) | The MCP resource identifier: both the audience an MCP token must carry and the `resource` value published in the [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) protected resource metadata. Unlike the REST audience this check is **mandatory** and cannot be disabled. Leaving it unset derives `<server public URL>/mcp` rather than turning it off. Set it only when your registered resource server uses a different identifier; it must then match that identifier, or MCP clients following the published metadata will be rejected |
| `server.security.management_api_key_hash` | `""` (empty) | SHA-256 hex digest of the management API key that authenticates calls to this deployment's import API (`/import`, `/import/**`) and variable store (`/variables`, `/variables/**`, `/secrets`, `/secrets/**`), so a deployment pipeline can push configuration without first obtaining OAuth client credentials. The key is presented in its own header, `API-Key: <key>`, never in `Authorization`. **This setting holds the digest, not the key**: the deployment only needs to recognize the key, so a leaked configuration file yields nothing a caller can present. Generate the pair with any SHA-256 tool, for example `printf '%s' "$KEY" | sha256sum`. A caller authenticates as a principal holding the **root system permission**, and is trusted **only on those paths**; every other API stays behind OAuth. An `API-Key` value that does not match is passed to the normal OAuth check, so OAuth tokens keep working on the same paths. Empty **disables it**, which is the default. Because it is a single long-lived key it cannot be scoped per caller or revoked without a restart, so treat it as a deployment credential, give it to as few callers as possible, and prefer an OAuth client wherever one can be obtained |
