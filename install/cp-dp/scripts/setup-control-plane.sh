...
# own resources, which is the one the platform APIs are served under. Key material is generated
# without a baseline, and the baseline seeded against that tenant instead.
#
# In server mode the deployment is single-tenant, and setup.sh seeds the baseline itself.
