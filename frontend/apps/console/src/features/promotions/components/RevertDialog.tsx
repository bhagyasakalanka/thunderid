...
/**
 * Confirms reverting an gateway to an earlier version.
 */
export default function RevertDialog({
  ...
  const handleRevert = (): void => {
    revert.mutate({gatewayId, toVersion, apply: applyNow}, {onSuccess: () => onClose()});
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>{t('promotions:revert.title', 'Revert gateway')}</DialogTitle>
      <DialogContent>
        <DialogContentText>
          {t('promotions:revert.message', 'Revert {{env}} to {{target}}?', {env: gatewayName, target})}
        </DialogContentText>
