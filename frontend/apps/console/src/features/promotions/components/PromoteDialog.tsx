...
import type {Diff, Gateway, ResourceChange} from '../models/promotion';
...
interface PromoteDialogProps {
  ...
  /**
   * Whether the target gateway's Data Plane is connected. A promotion writes to the target's
   * Control Plane and does not need it, but applying to the Data Plane afterwards does.
   */
  toDataPlaneConnected?: boolean;
  /** Version of the source gateway to promote. Defaults to its latest. */
  version?: string;
  onClose: () => void;
}

/**
 * Reviews a promotion before it happens: shows the diff against the target gateway, lets the
...
export default function PromoteDialog({
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetPromotionPreview(open ? fromGatewayId : '', open ? toGatewayId : '', version);
  const promote = usePromote();
  const {data: gateways} = useGetGateways();
  const {data: variableStatus} = useCheckVariables(open ? fromGatewayId : '', version);

  // What the user changed in this dialog, keyed by resource, holding whether they want it promoted.
  // Only their explicit toggles are tracked: everything else falls back to what the gateway
  ...
  const rememberedExclusions: Set<string> = useMemo(
    () => new Set(gateways?.gateways?.find((env: Gateway) => env.id === toGatewayId)?.excluded ?? []),
    [gateways, toGatewayId],
  );
  ...
  const handlePromote = (): void => {
    // The selection is always sent, never left to a default. It is what the gateway remembers for
      ...
      <DialogContent>
        <Typography variant="body2" color="text.secondary" sx={{mb: 2}}>
          {t('promotions:promote.description', 'Review what will change in {{target}}, taken from {{source}}.', {
            source: fromGatewayName,
            target: toGatewayName,
          })}
        </Typography>
