...
vi.mock('../../api/useGetGatewaySecrets', () => ({default: () => listResult}));
vi.mock('../../api/useGetGateways', () => ({
  default: () => ({data: {gateways: [{id: 'env-1', name: 'Dev'}]}}),
}));
vi.mock('react-router', async () => {
  const actual = await vi.importActual<typeof import('react-router')>('react-router');
  return {...actual, useParams: () => ({gatewayId: 'env-1'})};
});

describe('GatewaySecretsPage', () => {
  beforeEach(() => {
    listResult.data = secretList;
  });

  it('offers regenerate only for a credential the Data Plane hashes', () => {
    render(<GatewaySecretsPage />);
