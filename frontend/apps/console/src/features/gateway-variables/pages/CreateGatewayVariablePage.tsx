...
/**
 * Full-screen page for creating an gateway variable.
 */
export default function CreateGatewayVariablePage(): JSX.Element {
  const {t} = useTranslation();
  const navigate = useNavigate();
  const {gatewayId = ''} = useParams<{gatewayId: string}>();
  const createGatewayVariable = useCreateGatewayVariable(gatewayId);
  ...
  return (
    <Box sx={{maxWidth: 640, mx: 'auto', width: '100%', py: 6, px: 3}}>
      <Typography variant="h5" gutterBottom>
        {t('gatewayVariables:create.title', 'Add Gateway Variable')}
      </Typography>
      <Typography variant="body2" color="text.secondary" sx={{mb: 3}}>
        {t(
          'gatewayVariables:create.subtitle',
      ...
      <Stack spacing={3}>
        <Controller
          ...
          render={({field, fieldState}) => (
            <FormControl fullWidth>
              <FormLabel>{t('gatewayVariables:form.key.label', 'Key')}</FormLabel>
              <TextField
                {...field}
                ...
                helperText={
                  fieldState.error
                    ? t(
                        'gatewayVariables:form.key.invalid',
                        'Use letters, digits and underscores; it must not start with a digit.',
                      )
                    : t('gatewayVariables:form.key.help', 'The declarative placeholder name this variable resolves.')
                }
                fullWidth
              />
            </FormControl>
        ...
        <Stack direction="row" spacing={2} justifyContent="flex-end">
          ...
          <Button
            ...
            disabled={!isValid || createGatewayVariable.isPending}
          >
            {createGatewayVariable.isPending
              ? t('common:status.saving', 'Saving...')
              : t('gatewayVariables:create.submit', 'Create Variable')}
          </Button>
        </Stack>
      </Stack>
    </Box>
